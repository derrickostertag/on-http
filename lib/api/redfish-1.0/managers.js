...
var logger = Logger.initialize('manfred-debug');
var wsman = injector.get('Http.Services.Wsman');

var reservedId = 'RackHD';

function getReservedIdManager() {
     logger.info("***** RackHD found\n\n");
};

function getiDracManager() {
     logger.info("***** IDRAC found\n\n");
};

function getBMCManager() {
     logger.info("***** quanta device found\n\n");
};
...
var getManager = controller(function(req, res) {
    ...
    if (identifier === reservedId) {
        getReservedIdManager();
    } else if (wsman.isDellSystem(parts[0])) {
        getiDracManager();
    } else {
        getBMCManager();
    }
    ...
    .then(function(nodes) {
        ...
        logger.info("***** systems\n\n");
        if (wsman.isDellSystem(parts[0])) {
            logger.info("*****parts 0 works");
        } else {
            logger.info("*****parts 0 not working");
        }
        ...
        if(identifier !== reservedId) {
//        if((identifier !== reservedId) && (!wsman.isDellSystem(parts[0]))) {
            logger.info("***** non-dell and non rackhd");
